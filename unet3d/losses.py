...
# from DS_module.loss import MyBceloss12_n, MyWcploss
import numpy as np
...
class DiceLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # here, we skip first(background) layer
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        logpt = logpt.view(-1)
        logptf = logpt.data[:300000]
        logpts = logpt.data[300000:700000]
        logptt = logpt.data[700000:]

        ptf = torch.exp(logptf)
        pts = torch.exp(logpts)
        ptt = torch.exp(logptt)

        ptf = Variable(ptf)
        pts = Variable(pts)
        ptt = Variable(ptt)
        pt = torch.cat((ptf,pts,ptt),0)
        ...
        if self.size_average:
            lossf = loss.data[:300000].mean()
            losss = loss.data[300000:700000].mean()
            losst = loss.data[700000:].mean()
            loss_total = lossf + losss + losst
            loss_total.requires_grad = True
            return loss_total
        else:
            lossf = loss.data[:300000].sum()
            losss = loss.data[300000:700000].sum()
            losst = loss.data[700000:].sum()
            loss_total = lossf + losss + losst
            loss_total.requires_grad = True
            return loss_total
...
SUPPORTED_LOSSES = ['FocalLoss', 'BCEWithLogitsLoss', 'BCEWithLogitsLoss', 'CrossEntropyLoss', 'WeightedCrossEntropyLoss', 'PixelWiseCrossEntropyLoss',
                    'GeneralizedDiceLoss', 'DiceLoss', 'TagsAngularLoss', 'MSEWithLogitsLoss', 'MSELoss',
                    'SmoothL1Loss', 'L1Loss','DS_loss']
...
def get_loss_criterion(config):
    ...
    if name == 'BCEWithLogitsLoss':
        ...
        skip_last_target = loss_config.get('skip_last_target', True)
        ...
        return MyBceloss12_n(),MyWcploss()
    else:

...
class SliceBuilder:
    def __init__(self, raw_datasets, label_datasets, weight_dataset, patch_shape, stride_shape, phase, ignore_bg=True):
        # random ignore_bg for fast training
        if ignore_bg > random.random():
            ignore_bg = True
        else:
            ignore_bg = False
        self._raw_slices = self._build_slices(raw_datasets[0], patch_shape, stride_shape, phase, ignore_bg)
        if label_datasets is None:
            self._label_slices = None
        else:
            # take the first element in the label_datasets to build slices
            self._label_slices = self._build_slices(label_datasets[0], patch_shape, stride_shape, phase, ignore_bg)
    ...
    @staticmethod
    def _build_slices(dataset, patch_shape, stride_shape, phase, ignore_bg):
...
class ITKDataset(Dataset):
    ...
    def __init__(self, file_path, patch_shape, stride_shape, phase, transformer_config,
        ...
        if phase != 'test':
            ...
            if weight_internal_path is not None:
                ...
                self.weight_maps = [[internal_path][...] for internal_path in weight_internal_path]
        ...
        slice_builder = slice_builder_cls(
            self.raws, self.labels, self.weight_maps, patch_shape, stride_shape, phase, transformer_config['ignore_bg'])
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'test':
            ...
            if self.DS_module == 'fnd' or self.DS_module == 'dense_fnd':
                fnd_patch_transformed = self._transform_patches(self.fnd, label_idx, self.label_transform)
                return raw_patch_transformed, label_patch_transformed, fnd_patch_transformed
            else:

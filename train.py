...
# from datasets.itkDataset import get_train_loaders
from unet3d.config import load_config
from unet3d.losses import get_loss_criterion
from unet3d.metrics import get_evaluation_metric
from unet3d.model import get_model
from fpn3d.model import get_model_fpn
...
def _create_trainer(config, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders, logger):
    ...
    DS_module = loader_config.get('DS_module', None)
    ...
    if resume is not None:
        ...
        return UNet3DTrainer.from_checkpoint(resume, model,
                                             ...
                                             eval_criterion, loaders, DS_module,
                                             logger=logger)
        ...
        return UNet3DTrainer.from_pretrained(pre_trained, model, optimizer, lr_scheduler, loss_criterion,
                                             eval_criterion, checkpoint_path=trainer_config['checkpoint_dir'], device=config['device'], loaders=loaders,
    ...
    # , checkpoint_dir = trainer_config['checkpoint_dir']
        ...
        return UNet3DTrainer(model, optimizer, lr_scheduler, loss_criterion, eval_criterion,
                             config['device'], loaders, checkpoint_dir=trainer_config['checkpoint_dir'],
                             max_num_epochs=trainer_config['epochs'],
                             max_num_iterations=trainer_config['iters'],
                             validate_after_iters=trainer_config['validate_after_iters'],
                             log_after_iters=trainer_config['log_after_iters'],
...
def main():
    ...
    #logger = get_logger('HRNet3DTrainer')
    ...
    loss_criterion1 = get_loss_criterion(config)

    loss_config = config['loss']
    loss_config['name'] = 'GeneralizedDiceLoss'

    loss_criterion2 = get_loss_criterion(config)

    loss_criterion = [loss_criterion1, loss_criterion2]

    # loss_criterion1 , loss_criterion2 = get_loss_criterion(config)
    # loss_criterion = [loss_criterion1, loss_criterion2]
